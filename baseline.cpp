...
void CBaseline::allocate_memory()
{
	int i;
...
void CBaseline::write_parameters(FILE *fp)
{
...
void CBaseline::set_parameters(float para[])
{
	crossover_rate = para[0];
	scale_factor = para[1];
}

void CBaseline::init_baseline()
{
	init_random(seed);
}

void CBaseline::init_population_rand()
{
	int i, j;
...
void CBaseline::mutation(int cur_individual)
{
	int r1, r2, i;
...
void CBaseline::crossover(int cur_individual)
{
	int i;
	for (i=0; i<dimension; i++){
...
float CBaseline::evaluate(float *solu, int cur_node)
{
	float temp_fit;
...
void CBaseline::evaluate_init_population(int cur_node)
{
	int i;
	for (i=0; i<pop_size; i++){
...
void CBaseline::update_best_solution()
{
	int i, id_best=-1;
	for (i=0; i<pop_size; i++) {
		if (fit[i] < best_fit) {
...
void CBaseline::compose_best_solu(int cur_node)
{
	int i;
	int n_nodes = fcm->n_nodes;
	for (i=0; i<n_nodes; i++){
...
int CBaseline::get_iter_best_id()
{
	int i, id;
